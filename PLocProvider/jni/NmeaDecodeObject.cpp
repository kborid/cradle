/*=============================================================================
	...
	@(#)$Id: NmeaDecodeObject.cpp 25636 2011-05-04 09:07:25Z guoxianing $
...
static const INT NMEA_FIELD_DATA_SIZE = 32;			// NMEA recode field data size
	...
	m_iSatellite(0)
{
	...
	// Get Longitude
	// In field 2
	if (GetField(pStrData, 2, pFieldData) == TRUE)
	{
	...
	// Get Longitude N/S
	// In field 3
	if (GetField(pStrData, 3, pFieldData) == TRUE)
	{
		if (pFieldData[0] != 'N')
		{
			if (pFieldData[0] == 'S')
	...
	// Get Latitude
	// In field 4
	if (GetField(pStrData, 4, pFieldData) == TRUE)
	{
	...
	// Get Latitude E/W
	// In field 5
	if (GetField(pStrData, 5, pFieldData) == TRUE)
	{
		if (pFieldData[0] != 'E')
		{
			if (pFieldData[0] == 'W')
	...
	// In field 7
	if (GetField(pStrData, 7, pFieldData) == TRUE)
	{
	...
	// Get altitude
	// In field 9
	if (GetField(pStrData, 9, pFieldData) == TRUE)
	{
	...
	m_fAngle(0.0F)
{
	...
	// In field 1
	if (GetField(pStrData, 1, pFieldData) == TRUE)
	{
		// Hour
	...
	// Get direction
	// In field 8
	if (GetField(pStrData, 8, pFieldData) == TRUE)
	{
		// Direction
	...
	m_byDimension(0)
{
	...
	{
		if (GetField(pStrData, iFieldCnt, pFieldData) == TRUE)
		{
	...
	// Get PDOP
	// In field 15
	if (GetField(pStrData, 15, pFieldData) == TRUE)
	{
	...
	// Get HDOP
	// In field 16
	if (GetField(pStrData, 16, pFieldData) == TRUE)
	{
	...
	// Get VDOP
	// In field 17
	if (GetField(pStrData, 17, pFieldData) == TRUE)
	{
	...
	m_iSatInfoCnt(0)
{
	...
	else
	{
		...
		for (INT iInfoCnt = 0; iInfoCnt < ((iFieldTotalCnt - 5) / 4); iInfoCnt++)
		{
			// PRN code (01 ~ 32)
			// In field 4
			if (GetField(pStrData, 4 + iInfoCnt * 4, pFieldData) == TRUE)
			{
			...
			// In field 5
			if (GetField(pStrData, 4 + iInfoCnt * 4 + 1, pFieldData) == TRUE)
			{
			...
			// In field 6
			if (GetField(pStrData, 4 + iInfoCnt * 4 + 2, pFieldData) == TRUE)
			{
			...
			// In field 7
			if (GetField(pStrData, 4 + iInfoCnt * 4 + 3, pFieldData) == TRUE)
			{
